...
<head>
    ...
    <meta name="description"
        content="{{ $metaDescription ?? View::getSection('tenant_name', config('app.name')) . ' - 最新まとめ記事アンテナサイト' }}">
    ...
    @if (isset($app) && $app instanceof \App\Models\App)
        <link rel="alternate" type="application/rss+xml" title="{{ $app->name }} RSS Feed"
            href="{{ route('front.rss.app', $app) }}" />
    @else
        <link rel="alternate" type="application/rss+xml" title="ゆにこーんアンテナ 横断 RSS Feed"
            href="{{ route('front.rss.index') }}" />
    @endif
...
<body class="min-h-dvh bg-surface text-text-primary antialiased dark:bg-surface-dark dark:text-white"
    x-data="{
    ...
    {{-- Header --}}
    <header
        class="sticky top-0 z-40 border-b border-border/60 bg-surface-elevated/80 backdrop-blur-xl dark:border-border-dark/60 dark:bg-surface-elevated-dark/80">
        <div class="mx-auto flex h-12 max-w-5xl items-center justify-between px-4">
            {{-- Site name --}}
            <a href="{{ url('/') }}"
                class="flex items-center gap-2 text-base font-bold tracking-tight transition-opacity hover:opacity-70">
                <img src="{{ asset('images/icon.avif') }}" alt=""
            ...
            {{-- Desktop nav --}}
            <nav
                class="hidden items-center gap-5 text-sm font-medium text-text-secondary sm:flex dark:text-text-tertiary">
                <a href="{{ url('/') }}" class="transition-colors hover:text-text-primary dark:hover:text-white"
                    wire:navigate>ホーム</a>

                <a href="{{ route('front.sites-index') }}"
                    class="transition-colors hover:text-text-primary dark:hover:text-white" wire:navigate>登録サイト</a>
                <a href="{{ route('front.rss-index') }}"
                    class="transition-colors hover:text-text-primary dark:hover:text-white" wire:navigate>RSS一覧</a>
                <a href="{{ route('front.ranking') }}"
                    class="transition-colors hover:text-text-primary dark:hover:text-white" wire:navigate>ブログランキング</a>
                <a href="{{ route('front.apply') }}"
                    class="transition-colors hover:text-text-primary dark:hover:text-white" wire:navigate>相互リンク申請</a>
            ...
            <button type="button"
                ...
                @click="mobileMenuOpen = !mobileMenuOpen" aria-label="メニュー">
        ...
        <div
            class="border-t border-border/40 bg-surface-elevated/50 backdrop-blur-xl dark:border-border-dark/40 dark:bg-surface-elevated-dark/50">
            <div class="mx-auto max-w-5xl px-4">
                <nav
                    class="category-nav flex items-center gap-6 overflow-x-auto py-2.5 text-sm font-medium text-text-secondary dark:text-text-tertiary">
                    <a href="{{ url('/') }}"
                        class="shrink-0 transition-colors hover:text-text-primary dark:hover:text-white {{ request()->is('/') ? 'text-accent dark:text-accent font-bold' : '' }}"
                        wire:navigate>総合トップ</a>
                    @foreach ($activeApps as $appItem)
                        <a href="{{ route('front.home', $appItem) }}"
                            class="shrink-0 transition-colors hover:text-text-primary dark:hover:text-white {{ request()->route('app') === $appItem->api_slug ? 'text-accent dark:text-accent font-bold' : '' }}"
                            wire:navigate>{{ $appItem->name }}</a>
                    @endforeach
                </nav>
            </div>
        </div>
        ...
        <div x-show="mobileMenuOpen" x-cloak x-transition:enter="transition ease-out duration-200"
            ...
            <nav class="flex flex-col gap-1">
                <a href="{{ url('/') }}"
                    class="rounded-lg px-3 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/10"
                    wire:navigate>ホーム</a>

                <a href="{{ route('front.sites-index') }}"
                    class="rounded-lg px-3 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/10"
                    wire:navigate>登録サイト</a>
                <a href="{{ route('front.rss-index') }}"
                    class="rounded-lg px-3 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/10"
                    wire:navigate>RSS一覧</a>
                <a href="{{ route('front.ranking') }}"
                    class="rounded-lg px-3 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/10"
                    wire:navigate>ブログランキング</a>
                <a href="{{ route('front.apply') }}"
                    class="rounded-lg px-3 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/10"
                    wire:navigate>相互リンク申請</a>
                <a href="{{ route('front.about') }}"
                    class="rounded-lg px-3 py-2.5 text-sm font-medium transition-colors hover:bg-black/5 dark:hover:bg-white/10"
                    wire:navigate>このサイトについて</a>
    ...
    {{-- Footer --}}
    <footer
        class="border-t border-border/40 bg-surface-elevated/50 dark:border-border-dark/40 dark:bg-surface-elevated-dark/50">
        <div class="mx-auto max-w-5xl px-4 py-6">
            <div class="flex flex-col items-center gap-3 text-xs text-text-secondary dark:text-text-tertiary">
                <div class="flex flex-wrap justify-center gap-4">
                    <a href="{{ url('/') }}"
                        class="transition-colors hover:text-text-primary dark:hover:text-white">ホーム</a>
                    <a href="{{ route('front.sites-index') }}"
                        class="transition-colors hover:text-text-primary dark:hover:text-white">登録サイト</a>
                    <a href="{{ route('front.rss-index') }}"
                        class="transition-colors hover:text-text-primary dark:hover:text-white">RSS一覧</a>
                    <a href="{{ route('front.ranking') }}"
                        class="transition-colors hover:text-text-primary dark:hover:text-white">ランキング</a>
                    <a href="{{ route('front.apply') }}"
                        class="transition-colors hover:text-text-primary dark:hover:text-white">相互リンク申請</a>
                    <a href="{{ route('front.about') }}"
                        class="transition-colors hover:text-text-primary dark:hover:text-white">このサイトについて</a>
                </div>
