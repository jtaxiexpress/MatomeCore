    ...
    <div class="mb-8 overflow-hidden rounded-2xl border border-border/40 bg-surface-elevated shadow-sm dark:border-border-dark/40 dark:bg-surface-elevated-dark">
        ...
        <div class="divide-y divide-border/40 dark:divide-border-dark/40">
            @foreach($hotEntries as $index => $article)
                <a x-show="!mutedSites.includes({{ $article->site_id ?? 'null' }})" href="{{ route('front.go', ['app' => $article->app->api_slug, 'article' => $article->id]) }}" target="_blank" rel="noopener noreferrer" class="group flex items-start gap-4 p-4 transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">
